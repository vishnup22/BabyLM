...
def full_train_loop(cfg, accelerator, model, optimizer, scheduler, dataloader):
    # Start the loop
    start_time = time()
    ...
    for epoch in range(cfg["n_epochs"]):
        ...
        tr_metrics = train_epoch(cfg, accelerator, model, optimizer, scheduler, dataloader, epoch, epoch_size, start_time, checkpoint_steps)
        if accelerator.is_main_process:
            ...
            save_epoch_checkpoint(accelerator.unwrap_model(model), optimizer, scheduler, epoch, checkpoint_dir)
...
def train_epoch(cfg, accelerator, model, optimizer, scheduler, dataloader, epoch, epoch_size, start_time, checkpoint_steps):
    model.train()
    total_loss = 0
    total_tokens = 0
    temp_loss = 0
    ...
    for train_step, minibatch in enumerate(tqdm(dataloader, disable=not accelerator.is_local_main_process)):
        ...
        if cfg["gradient_clip_norm"] != -1: 
            accelerator.clip_grad_norm_(model.parameters(), cfg['gradient_clip_norm'])
        optimizer.step()
        scheduler.step()
        optimizer.zero_grad()

        gathered_loss = accelerator.gather_for_metrics(loss.detach().unsqueeze(0)).mean().item()
        gathered_tokens = accelerator.gather_for_metrics(
            torch.tensor([num_tokens], device=device, dtype=torch.long)
        ).sum().item()

        total_loss += gathered_loss * gathered_tokens
        total_tokens += gathered_tokens
        temp_loss += gathered_loss * gathered_tokens
        temp_tokens += gathered_tokens

        if accelerator.is_main_process and cfg["use_wandb"] and (train_step % 10 == 0 and train_step > 0):
            # Compute the steps
        ...
        if accelerator.is_main_process and global_step in checkpoint_steps:
            label = checkpoint_steps[global_step]
            print(f"\n  Saving checkpoint at {label} words (step {global_step})")
            save_epoch_checkpoint(accelerator.unwrap_model(model), optimizer, scheduler, label, cfg["checkpoint_dir"])
...
def main():
    ...
    if accelerator.is_main_process:
        print("Env init")
        print(f"Using accelerate with {accelerator.num_processes} process(es) on device {accelerator.device}")
    ...
    # Perform training
    full_train_loop(cfg, accelerator, model, optimizer, scheduler, dataloader)
