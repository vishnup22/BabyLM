"""
Bilingual eng-hin evaluation.

Models:
  gpt2_bilingual : pulipakav-1/gpt2-hin-eng_babylm2026
...
Evaluations (Hindi):
  - hi_perplexity : pulipakav-1/translated-babylm-hindi  (split=test)
  - hi_mblimp     : jumelet/multiblimp  (config=hin)
  - hi_sib200     : Davlan/sib200  (config=hin_Deva)
  - hi_mubench    : per-task configs, lang suffix _hi

Usage:
    python eng_hin.py --cleaned_dir /path/to/cleaned
    python eng_hin.py --evals en_perplexity en_blimp hi_perplexity hi_mblimp
    python eng_hin.py --output results_enghin.json
"""
...
MODELS = {
    "gpt2_bilingual": {
        "model_id": "pulipakav-1/gpt2-hin-eng_babylm2026",
        "arch": "causal",
    },
}
    ...
    if cleaned_dir:
        ...
        for row in ds:
            ...
            if max_samples and len(texts) >= max_samples:
                break
    ppl = compute_perplexity(model, tokenizer, arch, texts, device, extras, max_seq_len)
    print(f"    [HI] Perplexity [test]: {ppl:.4f}")
    return {"test": round(ppl, 4)}
...
def eval_hi_mblimp(model, tokenizer, arch, device, extras) -> dict:
...
def main():
    parser = argparse.ArgumentParser(description="Bilingual eng-hin evaluation")
    ...
    parser.add_argument("--cleaned_dir", default=None,
                        help="Directory with pre-cleaned .txt files (english_test.txt, hindi_test.txt)")
    parser.add_argument("--max_ppl_samples", type=int, default=0,
                        help="Max sentences for perplexity (0 = all)")
    parser.add_argument("--max_seq_len", type=int, default=128)
    parser.add_argument("--output", default="results_enghin.json")
    ...
    for name in args.models:
        ...
        # ── Hindi evals ────────────────────────────────────────────────────────
        if "hi_perplexity" in args.evals:
        ...
        if "hi_mblimp" in args.evals:
            save_eval("hi_mblimp", name,
                      eval_hi_mblimp(model, tokenizer, arch, device, extras))

        if "hi_sib200" in args.evals:
            save_eval("hi_sib200", name,
                      eval_sib200(model, tokenizer, arch, device, extras,
                                  "hin_Deva", "विषय: "))

        if "hi_mubench" in args.evals:
            save_eval("hi_mubench", name,
                      eval_mubench(model, tokenizer, arch, device, extras,
                                   args.mubench_dataset, MUBENCH_TASKS_HI, "hi"))
