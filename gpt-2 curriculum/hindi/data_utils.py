...
def resolve_dataset_dir(dataset_name: str, data_root: Path = Path('data')) -> Path:
    """Use the dataset root if files are there, otherwise fall back to a train/ subfolder."""
...
def load_texts_from_dir(data_dir: Path) -> list[tuple[str, str]]:
    ...
    for f in sorted(data_dir.glob('*.train*.txt')):
        source = source_name_from_path(f)
        text = f.read_text()
...
class FullBabyLMDataset(Dataset):
    ...
    def __init__(self, cfg):
        ...
        if curriculum:
            curriculum_file = Path(cfg['curriculum_file'])
            print(f'Curriculum mode: loading {curriculum_file}')
            all_text = curriculum_file.read_text(encoding='utf-8')
            tokenized = self.processor(text=[all_text], add_special_tokens=False)['input_ids'][0]
            print(f'Tokenized curriculum; {len(tokenized):,} tokens total')
            num_chunks = math.ceil(len(tokenized) / chunk_size)
            for curr_chunk in tqdm(range(num_chunks)):
                start = curr_chunk * chunk_size
                end = (curr_chunk + 1) * chunk_size
                self.data.append(tokenized[start:end])
            print(f"Chunked curriculum file")
        else:
            data_dir = resolve_dataset_dir(dataset_name)
            ...
            for source_name, all_text in texts:
                ...
                for curr_chunk in tqdm(range(num_chunks)):
                    ...
                    self.data.append(tokenized_dataset[start:end])
...
def load_babylm_data(cfg):
    ...
    filename = cache_dir / f'train_gpt2_{dataset_name}{suffix}.pkl'
    ...
    dataloader = DataLoader(full_babylm_dset, batch_size=cfg["batch_size"],
                            shuffle=not curriculum, collate_fn=collate_fn)
    return dataloader
