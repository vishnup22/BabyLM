"""
Small model (GPT-Wee, 28M) Hindi evaluation.
Models: pulipakav-1/babylm_hindi_2026 — random_seed{1,2,3} and curriculum_seed{1,2,3}
...
Usage:
    ...
    python hindi.py --models random_seed1 curriculum_seed1
    python hindi.py --evals perplexity mblimp
    python hindi.py --output results_small_hindi.json
"""
...
MODELS = {
    "random":     {"model_id": "pulipakav-1/gpt2-hindi-small-babylm2026",            "subfolder": None, "arch": "causal"},
    "curriculum": {"model_id": "pulipakav-1/gpt2-hindi-small-curriculum_babylm2026", "subfolder": None, "arch": "causal"},
}
...
def eval_perplexity(model, tokenizer, device, cleaned_dir=None, max_samples=None, max_seq_len=128) -> dict:
    if cleaned_dir:
        texts = (Path(cleaned_dir) / "hindi_test.txt").read_text(encoding="utf-8").splitlines()
...
def eval_mblimp(model, tokenizer, device) -> dict:
    ...
    for row in tqdm(ds, desc="  M-BLiMP", leave=False):
        good = score_causal(model, tokenizer, row["sen"],       device)
        bad  = score_causal(model, tokenizer, row["wrong_sen"], device)
        if good > bad:
            correct += 1
    acc = correct / len(ds)
...
def main():
    parser = argparse.ArgumentParser(description="Small model Hindi evaluation")
    ...
    parser.add_argument("--output", default="results_small_hindi.json")
    ...
    for name in args.models:
        ...
        if "mblimp" in args.evals:
            save_eval("mblimp", name, eval_mblimp(model, tokenizer, device))
