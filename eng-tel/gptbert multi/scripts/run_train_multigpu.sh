...
DATASET_SOURCE_DIR="${DATASET_SOURCE_DIR:-$ROOT_DIR/../gpt2 multi/data/en_tel_equal}"
DATA_BASE_DIR="${DATA_BASE_DIR:-$ROOT_DIR/data/EN_TEL_EQUAL}"
...
TOK_FILE="${TOKENIZER_PATH:-$TOK_DIR/tokenizer_en_tel_vs${VOCAB_SIZE}.json}"
...
if [[ $need_preprocess -eq 1 ]]; then
  ...
  if [[ ! -f "$TOK_FILE" ]]; then
    ...
    python "$ROOT_DIR/tokenizers/tokenizer.py" \
      ...
      --output "tokenizer_en_tel_vs${VOCAB_SIZE}.json"
  else
