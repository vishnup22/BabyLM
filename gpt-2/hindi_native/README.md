# Hindi BabyLM Strict GPT-2 (native, non-translated data)

This folder is a self-contained copy of the `strict-gpt2` training path, recovered from git history (the original translated-data version was removed as "legacy") and adapted to train on **native Hindi text sourced from CC-100** instead -- deduplicated and byte-matched to the translated corpus's size so the two are comparable. Same training method as `../hindi/` (translated-data GPT-2), only the data source differs -- mirrors the same native/translated split already set up for GPT-BERT (`../../gpt-bert/hindi_native/`).

It is set up to train a GPT-2 model from scratch on:

- Hugging Face dataset: `pulipakav-1/hi-te` (`hindi.txt`)
- Downloaded fresh by `scripts/download_dataset.sh` into `data/native-babylm-hindi/native.train.hi.txt`
...
- `data/native-babylm-hindi/`
- `tokenizers/native-babylm-hindi/`
- `configs/native-babylm-hindi/`
- `experiments/hindi-native-strict-100m/`
- `pulipakav-1/hindi-native-gpt2-base` on Hugging Face (pushed automatically at the end of `run_all.sh` -- no manual step needed once the job finishes)
...
Or run everything with (used by `hindi_native.sh`'s SLURM job):
...
- `training.py` expects the tokenizer under `./tokenizers/native-babylm-hindi` and the model config under `./configs/native-babylm-hindi`.
...
- Trained with a single seed (no `--seed` override, matching `../hindi/`'s single-run launcher) rather than the seed-1/seed-2 pattern used by `../telugu/`'s original launcher -- kept consistent with the single-seed choice already made for the native GPT-BERT bundles, given the 5-day time budget.
