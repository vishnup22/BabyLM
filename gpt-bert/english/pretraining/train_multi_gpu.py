...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        help="Disable validation and train on the full prepared corpus.",
    )
    parser.add_argument(
...
def training_epoch(
    ...
    for local_step in range(num_steps):
        ...
        if (
            ...
            and (global_step + 1) % args.validate_every == 0
        ):
...
def load_datasets(
    ...
    if valid_dataloader is None and not args.no_validation and args.valid_path:
...
if __name__ == "__main__":
    ...
    save(model, ema_model, optimizer, scheduler, global_step, epoch, args)
    if valid_dataloader is not None and not args.no_validation:
